...
// Copied from OVRLipSyncContextTextureFlip but outputs the right texture as an API instead.
public class VismesBest : MonoBehaviour {
    // PUBLIC

    private string[] vismesNames = new string[] { "sil", "PP", "FF", "TH", "DD", "kk", "CH",
        "SS", "nn", "RR", "aa", "E", "ih", "oh", "ou" };

    public string CurrentVisme { get; private set; }
    ...
    void SetVisemeToTexture() {
        ...
        if ((gV != -1) && (gV < vismesNames.Length)) {
            CurrentVisme = vismesNames[gV];
        }
    }
}
